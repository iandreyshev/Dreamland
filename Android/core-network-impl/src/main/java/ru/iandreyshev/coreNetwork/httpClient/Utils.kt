...
internal fun getRequestTo(path: String, account: AccountIdentifier): Request =
        Request.Builder()
                .get()
                .url(MOBILE_API_URL + path)
...
internal fun <T> apiCall(request: Request, connError: T, serverError: T, onSuccess: (String) -> T): T {
    return try {
